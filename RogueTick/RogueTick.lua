...
-- =============================================
-- MODULE: Pro Stats HUD (Strict Monochrome Edition)
-- =============================================

local hud = CreateFrame("Frame", "ET_StatsHUD", UIParent, BackdropTemplateMixin and "BackdropTemplate" or nil)
hud:SetSize(280, 48) 
...
hud:SetScript("OnDragStart", hud.StartMoving); hud:SetScript("OnDragStop", hud.StopMovingOrSizing)

-- Темный фон для контраста со светло-серыми буквами
hud:SetBackdrop({
...
hud:SetBackdropColor(0, 0, 0, 0.75); hud:SetBackdropBorderColor(0.15, 0.15, 0.15, 1)

-- Функция создания монохромной ячейки
local function CreateDataCell(headerText, xOff)
    local mainFont = "Fonts\\ARIALN.TTF" 
    local lightGray = {0.85, 0.85, 0.85} -- Светло-серый цвет

    -- Заголовок (Label)
    local h = hud:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    h:SetFont(mainFont, 9, "OUTLINE")
    h:SetPoint("TOPLEFT", hud, "TOPLEFT", xOff, -2) -- Максимально вверх
    h:SetText(headerText)
    h:SetTextColor(0.5, 0.5, 0.5) -- Чуть темнее серого для подписей

    -- Значение (Value)
    local v = hud:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    v:SetFont(mainFont, 16, "OUTLINE")
    v:SetPoint("TOPLEFT", h, "BOTTOMLEFT", 0, -4) 
    v:SetTextColor(unpack(lightGray)) -- Фиксированный светло-серый
    return v
end

-- Латинские заголовки
local vMs   = CreateDataCell("TIMING", 12)
local vEn   = CreateDataCell("ENERGY", 75)
local vCP   = CreateDataCell("POINTS", 135)
local vHP   = CreateDataCell("HEALTH", 185)
local vSnD  = CreateDataCell("SND", 245)

hud:SetScript("OnUpdate", function(self, elapsed)
    -- Energy
    vEn:SetText(UnitPower("player", 3))

    -- Combo Points
    vCP:SetText(UnitPower("player", 4))

    -- Health
    local hp, mhp = UnitHealth("player"), UnitHealthMax("player")
    vHP:SetText(math.floor((hp/(mhp > 0 and mhp or 1))*100))

    -- Slice and Dice
    local sndTime = 0
    for i = 1, 40 do
        local name, _, _, _, _, exp = UnitBuff("player", i)
    ...
    vSnD:SetText(sndTime > 0 and math.floor(sndTime) or "0")
end)

-- Timing
local msFrame = CreateFrame("Frame")
msFrame:RegisterEvent("UNIT_SPELLCAST_SENT")
msFrame:SetScript("OnEvent", function(_, _, unit)
    if unit == "player" then
        local now = GetTime()
        local _, _, _, lat = GetNetStats()
        local delay = (EnergyTickSettings and EnergyTickSettings.delay) or 0.4
        local diff = 2 - (now - (lastT or now)) - (lat/1000)
        local ms = (diff - delay) * 1000
        
        vMs:SetText(string.format("%.0f", ms))
    end
end)